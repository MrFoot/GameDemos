 ...
 * 说一下这个锁的用意：
 * constructor里面如果发现loop=null，不自己创建，而是先wait（会暂时释放锁、并进入等待队列），然后等待Run线程创建loop，再通过pulse唤醒主线程（使主线程从等待队列，进入就绪队列——注意，还是卡住的状态，需要等待Run线程释放了lock，才会走到下一步）
 * 总结一下：
 * 如果constructor先获得锁，又发现loop=null，就是wait，进入休眠并暂时释放锁。run获得锁，创建了loop，然后pulse唤醒了constructor的线程，然后释放了锁。constructor获得锁，从就绪队列进入执行队列，继续。
